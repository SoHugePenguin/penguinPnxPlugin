...
public class KeepFill extends FormWindowCustom {
    ...
    public static void KeepFill_do(int id, int special_id, Player player) {
        String text = "";
        if (BlockState.of(id, special_id).getBlock().getName() != null) {
            if (BlockState.of(id, special_id).getBlock().getName().contains("UNKNOWN")) {
                special_id = 0;
...
//            从小到大排序
            ...
            if (vector3s.size() == 2) {
                ...
                if (all_ok) {
                    ...
                    player.sendMessage("§e坐标1 " + Arrays.toString(xyz1) + "\n§a坐标2 " + Arrays.toString(xyz2) + " §6 属性: " + BlockState.of(id, special_id));
                    long x_fill = xyz1[0];
                    long y_fill = xyz1[1];
                    long z_fill = xyz1[2];
//                    刷新undo撤销
                    ...
                    while (true) {
                        ...
                        if (player.getLevel().getBlock(vector_fill).getId() == 0) {
                            ...
                            player.level.setBlock(vector_fill, BlockState.of(id, special_id).getBlock());
                            ++end_all;
                        }
