...
public class Paste_Build extends FormWindowSimple {
    ...
    public static void PasteBuild(Player p, String button, int way) {
        ...
        BuildInfo buildInfo = YmlTool.getYml(new File(button), BuildInfo.class);

        String location = buildInfo.getLocation();
        String block = buildInfo.getBlock();

        String[] location_list = location.split(" ");
        String[] block_list = block.split(" ");
        double x = p.getX(), y = p.getY(), z = p.getZ();
        double max_x = buildInfo.getMax_x(), max_y = buildInfo.getMax_y(), max_z = buildInfo.getMax_z();
        int j = 0;
